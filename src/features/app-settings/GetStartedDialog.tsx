...
//
// A11Y-DIALOG (migrerad till delad <Modal> T33): knappen (i GetStartedControl) är en
// <button> med aria-haspopup="dialog". Dialog-kontraktet (role="dialog" + aria-modal,
// aria-labelledby + aria-describedby, Escape, bakgrundsklick, fokus in/ut, fokus-fälla,
// portal till body, motion-gating) ägs nu av den delade <Modal>-primitiven (T33/#56),
// som GetStartedControl renderar runt detta innehåll. VIKTIGT: guiden kan öppnas OVANPÅ
// onboardingen, så GetStartedControl sätter escapeCapture på <Modal> (capture-fas +
// stopPropagation), så Escape bara stänger guiden, inte den underliggande touren
// (T54/#93 F2, bevarat). Denna komponent bidrar med stäng-knappen + hero + innehållet.
...
// Dialogens aria-id:n, en sanning som BÅDE <Modal>-wrappern (i GetStartedControl, för
// aria-labelledby/-describedby) OCH innehållet här (rubrik-/intro-elementens id) delar,
// så de aldrig kan drifta isär.
export const GET_STARTED_HEADING_ID = 'kom-igang-rubrik';
export const GET_STARTED_INTRO_ID = 'kom-igang-intro';

/** Props: dialogen styrs utifrån (öppen/stäng), så triggern äger fokus-återställningen. */
export interface GetStartedDialogProps {
  /** Stäng-callback (stäng-knappen anropar denna; <Modal> äger Escape/bakgrundsklick). */
  onClose: () => void;
  /** Ref till stäng-knappen (fokus-startpunkt vid öppning, satt av <Modal>). Ägs av triggern. */
...
/**
 * Kom-igång-dialogens INNEHÅLL (renderas inuti den delade <Modal> i GetStartedControl).
 * Härleder plattforms-läget EN gång per öppning (rena detektor-anrop via
 * resolveGetStartedState) och låter användaren byta väg via flikar.
 */
export function GetStartedDialog({
  ...
  const headingId = GET_STARTED_HEADING_ID;
  const introId = GET_STARTED_INTRO_ID;
    ...
    <>
      ...
      <div className="flex min-h-0 flex-1 flex-col overflow-y-auto">
        ...
        <header className="vm-get-started-hero relative flex items-center gap-4 overflow-hidden border-b border-border px-6 pb-5 pr-14 pt-6 sm:px-7 sm:pt-7">
          ...
          <span aria-hidden="true" className="vm-hero-sheen pointer-events-none absolute inset-0" />
