...
// ---- JOKER-VÄLJAREN (T19, #19) -----------------------------------------------

describe('PredictionForm, joker-väljaren', () => {
  const jokerToggle = () => screen.getByRole('button', { name: /joker/i });

  it('utan joker-prop visas INGEN joker-knapp (fixtures/icke-rum-läge oförändrat)', () => {
    render(
      <PredictionForm
        match={MATCH}
        teamsById={teamsById}
  ...
  it('öppen kupong: visar "gör till din joker" och anropar onToggle vid klick', async () => {
    ...
    render(
      <PredictionForm
        ...
        locked={false}
        onSubmit={vi.fn()}
        joker={{ isJoker: false, otherJokerSameDayMatchLabel: null, onToggle }}
      />
    );
    const btn = jokerToggle();
    expect(btn).toHaveTextContent(/Gör till din joker/);
    expect(btn).toHaveAttribute('aria-pressed', 'false');
    fireEvent.click(btn);
    await waitFor(() => expect(onToggle).toHaveBeenCalledTimes(1));
  });

  it('aktiv joker: knappen visar "din joker idag" + aria-pressed=true + data-joker-active', () => {
    render(
      <PredictionForm
        match={MATCH}
        teamsById={teamsById}
        ...
        locked={false}
        onSubmit={vi.fn()}
        joker={{ isJoker: true, otherJokerSameDayMatchLabel: null, onToggle: vi.fn() }}
      />
    );
    const btn = jokerToggle();
    expect(btn).toHaveTextContent(/Din joker idag/);
    expect(btn).toHaveAttribute('aria-pressed', 'true');
    expect(btn).toHaveAttribute('data-joker-active', 'true');
  });

  it('joker på en ANNAN match samma dag: visar flytta-hint', () => {
    render(
      <PredictionForm
        match={MATCH}
        teamsById={teamsById}
        ...
        joker={{
          isJoker: false,
          otherJokerSameDayMatchLabel: 'Brasilien mot Argentina',
          onToggle: vi.fn(),
        }}
      />
    );
    expect(screen.getByText(/Du har jokern på Brasilien mot Argentina idag/)).toBeInTheDocument();
  });

  it('FAIL LOUD: ett fel vid joker-toggle visas inline (RLS-avslag/nätfel)', async () => {
    const onToggle = vi.fn().mockRejectedValue(new Error('Spara joker misslyckades: låst'));
    render(
      <PredictionForm
        match={MATCH}
        teamsById={teamsById}
    ...
    fireEvent.click(jokerToggle());
    expect(await screen.findByRole('alert')).toHaveTextContent(/Spara joker misslyckades/);
  });

  it('LÅST kupong som var min joker: ingen knapp, men en "jokern gällde här"-markör', () => {
    render(
      <PredictionForm
    ...
    // Ingen toggle-KNAPP på en låst kupong (jokern är bindande efter avspark).
    expect(screen.queryByRole('button', { name: /joker/i })).not.toBeInTheDocument();
    // Men markören som visar att poängen dubblades.
    expect(screen.getByText(/Joker, poängen dubblades/)).toBeInTheDocument();
  });
});
