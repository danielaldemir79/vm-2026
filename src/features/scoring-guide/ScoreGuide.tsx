...
//
// A11Y-DIALOG (migrerad till delad <Modal> T33): knappen är en <button> med
// aria-haspopup="dialog"/aria-expanded. Dialog-kontraktet (role="dialog", aria-modal,
// aria-labelledby/-describedby, Escape, bakgrundsklick, fokus in/ut, fokus-fälla, portal
// till body) ägs nu av den delade <Modal>-primitiven (T33/#56). ScoreGuide bidrar med
// sitt INNEHÅLL (hero + förklaringen) + den distinkta overlay-/panel-stilen via slottarna.
//
// SURFACE-NAMNRYMD: de TVÅ mount-punkterna (tippning, topplista) får egna data-krokar
// via ett VÄRDE i attributen (`data-score-guide-overlay={surface}`, `-panel={surface}`),
// som <Modal> stödjer via overlayValue/panelValue. aria-id:na id-saniteras (toIdSafeSurface)
// och skickas in som labelledById/describedById.
...
export interface ScoreGuideProps {
   ...
   * (`data-score-guide-open={surface}`, `-overlay`, `-panel`, `-close`), så de TVÅ
...
export function ScoreGuide({ surface = 'tips' }: ScoreGuideProps) {
  ...
  return (
    <>
      ...
      {open ? (
        <ScoreGuideDialog surface={surface} closeButtonRef={closeButtonRef} onClose={close} />
      ) : null}
    </>
  );
}

/** Förklarings-dialogen ovanpå <Modal>-primitiven: hero + intro + sektionerna. Portal/
 * Escape/fokus/fokus-fälla/motion ägs av <Modal>. Fokus in till stäng-knappen. */
