...
function pointTypeOf(predicted, actual) {
  if (isExact(predicted, actual)) {
    return "exact";
  }
  if (outcomeOf2(predicted) === outcomeOf2(actual)) {
    return "outcome";
  }
  return "miss";
}
...
// src/data/predictions/bonus-score.ts
function normalizeTeamRef(teamRef) {
  return teamRef.toUpperCase();
}
function sameTeam(a, b) {
  return normalizeTeamRef(a) === normalizeTeamRef(b);
}
var GROUP_PREDICTION_POINTS = {
  /** Rätt gruppvinnare (1:a). */
  winner: 3,
  /** Rätt grupptvåa (2:a). */
  runnerUp: 2
};
function scoreGroupPrediction(predicted, actual) {
  let points = 0;
...
var CHAMPION_PREDICTION_POINTS = 20;
function scoreBracketAdvance(stage, predicted, actual) {
  return sameTeam(predicted, actual) ? BRACKET_ROUND_POINTS[stage] : 0;
}
function scoreChampionPrediction(predictedChampion, actualChampion) {
  return sameTeam(predictedChampion, actualChampion) ? CHAMPION_PREDICTION_POINTS : 0;
}

// src/data/predictions/bracket-predictions-api.ts
var CHAMPION_SLOT_ID = "champion";

// src/features/leaderboard/derive-facit.ts
function buildIdToCode(teams) {
...
function deriveMatchFacit(matches) {
  const facit = [];
...
var MATCHES_PER_TEAM2 = 3;
function deriveGroupFacit(groups, matches, idToCode) {
  const facit = [];
  for (const group of groups) {
    const standings = computeStandings(group.teamIds, matches);
    const complete = standings.length > 0 && standings.every((row) => row.played >= MATCHES_PER_TEAM2);
    if (!complete) {
      continue;
    }
...
function deriveBracketFacit(groups, matches, idToCode) {
  ...
  for (const matchState of bracket.matches) {
    ...
    const winnerSlot = matchState.home.id === matchState.winnerSlotId ? matchState.home : matchState.away;
...
function mapTeamId(teamId2, idToCode) {
  const code = idToCode.get(teamId2);
  if (code === void 0) {
    throw new Error(
      `[VM2026] Facit refererar lag-id "${teamId2}" som saknar en code i lag-listan (brutet referens-kontrakt). Kan inte po\xE4ngs\xE4tta tips mot ett ok\xE4nt lag.`
    );
  }
  return code;
}
function derivePoolFacit(teams, groups, matches) {
...
// src/features/leaderboard/aggregate-scores.ts
var EMPTY_BY_SOURCE = { match: 0, group: 0, bracket: 0, champion: 0 };
function indexFacit(facit) {
  return {
...
function scoreMember(member, index) {
  let matchPoints = 0;
  let groupPoints = 0;
  let bracketPoints = 0;
...
function buildLeaderboard(members, predictionsByUser, facit) {
  const index = indexFacit(facit);
  const lines = members.map((member) => {
    const preds = predictionsByUser.get(member.userId);
    const score = preds ? scoreMember(preds, index) : { points: 0, exactHits: 0, bySource: EMPTY_BY_SOURCE };
    return {
...
// src/features/total-leaderboard/aggregate-total.ts
...
function accumulateBest(best2, room, facit) {
  const perRoom = buildLeaderboard(room.members, room.predictionsByUser, facit);
  for (const entry of perRoom) {
    const existing = best2.get(entry.userId);
    if (existing === void 0) {
      best2.set(entry.userId, {
        userId: entry.userId,
...
function assignTotalRanks(sorted) {
  const entries = [];
  let previousPoints = null;
  let currentRank = 0;
  sorted.forEach((best2, index) => {
    if (previousPoints === null || best2.points !== previousPoints) {
      currentRank = index + 1;
      previousPoints = best2.points;
    }
    entries.push({
      userId: best2.userId,
      displayName: best2.displayName,
      points: best2.points,
      rank: currentRank,
      exactHits: best2.exactHits
    });
  });
  return entries;
}
function buildTotalLeaderboard(rooms, facit) {
  const best2 = /* @__PURE__ */ new Map();
  for (const room of rooms) {
    accumulateBest(best2, room, facit);
  }
  const sorted = [...best2.values()].sort(compareTotals);
  return assignTotalRanks(sorted);
}

// src/data/global-leaderboard/build-global-leaderboard.ts
function buildGlobalFacit(plan, officialResults) {
...
function buildGlobalLeaderboard(rooms, officialResults, plan) {
  const facit = buildGlobalFacit(plan, officialResults);
  const contributions = rooms.map((room) => ({
    roomId: room.roomId,
